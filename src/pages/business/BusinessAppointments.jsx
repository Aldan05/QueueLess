...
import { FiCalendar, FiClock, FiCheckCircle, FiXCircle, FiSearch, FiAlertCircle, FiEye, FiX, FiShield, FiMove } from 'react-icons/fi';
...
const BusinessAppointments = () => {
  ...
  const executeDelete = async (aptId) => {
    try {
    ...
    } catch (error) { toast.error('Server error deleting appointment'); }
  };
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex justify-end gap-2 items-center h-full">
                        ...
                        {['cancelled', 'rejected', 'completed', 'no_show'].includes(apt.status) && (
                          <button onClick={() => setActionModal({ type: 'delete', aptId: apt._id })} className="px-3 py-1.5 bg-red-50 text-red-600 rounded-lg text-xs font-bold hover:bg-red-100 transition-colors">Delete</button>
                        )}
                      </div>
                    </td>
                  </tr>
          ...
          >
            ...
            >
              ...
              <div className="p-6 space-y-5 overflow-y-auto max-h-[calc(90vh-80px)] custom-scrollbar">
                ...
                {viewingAppointment.documents && viewingAppointment.documents.length > 0 && (
                  <div>
                    <p className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-2 flex items-center gap-2">
                      <FiShield className="text-blue-600" /> ID Verification
                    </p>
                    <div className="bg-orange-50 dark:bg-orange-900/20 border border-orange-100 dark:border-orange-800 p-4 rounded-2xl">
                      {viewingAppointment.documents.map((doc, idx) => (
                        <div key={idx} className="space-y-3">
                          <p className="font-bold text-orange-900 dark:text-orange-400">Document Type: {doc.type}</p>
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            {doc.frontImage && (
                              <div>
                                <p className="text-xs font-semibold text-orange-700 dark:text-orange-500 mb-1">Front Image</p>
                                <a href={doc.frontImage} target="_blank" rel="noreferrer">
                                  <img src={doc.frontImage} alt="Front ID" className="w-full h-36 object-cover rounded-xl border border-orange-200 hover:opacity-90 transition-opacity" />
                                </a>
                              </div>
                            )}
                            {doc.backImage && (
                              <div>
                                <p className="text-xs font-semibold text-orange-700 dark:text-orange-500 mb-1">Back Image</p>
                                <a href={doc.backImage} target="_blank" rel="noreferrer">
                                  <img src={doc.backImage} alt="Back ID" className="w-full h-36 object-cover rounded-xl border border-orange-200 hover:opacity-90 transition-opacity" />
                                </a>
                              </div>
                            )}
                          </div>
        ...
        {actionModal.type === 'delete' && (
          ...
          >
            <motion.div 
              ...
              exit={{ scale: 0.95, opacity: 0 }}
              onClick={(e) => e.stopPropagation()}
              className="bg-white dark:bg-slate-800 rounded-3xl w-full max-w-md p-6 shadow-2xl border border-gray-100 dark:border-slate-700"
            >
              <div className="flex items-center justify-between mb-4 cursor-grab active:cursor-grabbing select-none">
                <div className="flex items-center gap-2">
                  <FiMove className="text-gray-400 w-4 h-4" />
                  <h2 className="text-xl font-extrabold text-gray-900 dark:text-white">Confirm Deletion</h2>
                </div>
                <span className="text-[11px] font-bold px-2 py-0.5 bg-gray-100 dark:bg-slate-700 text-gray-500 dark:text-gray-400 rounded-md">Draggable</span>
              </div>
              <div className="p-4 bg-red-50 dark:bg-red-950/30 rounded-2xl border border-red-100 dark:border-red-900/40 mb-6 flex items-start gap-3">
                <FiAlertCircle className="w-6 h-6 text-red-600 dark:text-red-400 shrink-0 mt-0.5" />
                <div>
                  <p className="text-sm font-bold text-red-900 dark:text-red-200">Delete Appointment Record?</p>
                  <p className="text-xs text-red-700 dark:text-red-300/80 mt-1">This appointment record will be permanently deleted from the system. This action cannot be undone.</p>
                </div>
              </div>
              <div className="flex justify-end gap-3">
                <button onClick={() => setActionModal({type: null, aptId: null})} className="px-5 py-2.5 bg-gray-100 dark:bg-slate-700 rounded-xl font-bold text-gray-700 dark:text-gray-200 hover:bg-gray-200 dark:hover:bg-slate-600 transition-colors">
                  Cancel
                </button>
                <button onClick={() => executeDelete(actionModal.aptId)} className="px-5 py-2.5 bg-red-600 rounded-xl font-bold text-white hover:bg-red-700 shadow-md shadow-red-500/20 transition-all flex items-center gap-2">
                  Delete Record
                </button>
              </div>
            </motion.div>
